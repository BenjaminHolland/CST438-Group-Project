...
export class TeamMatchComponent implements OnInit {
  ...
  public usersTeams: any[] = []; //array of all taems the user is on
  public allTeams: any[] = [];	//every team in the database
  public opponents: any[] = [];	//valid opponents for this users team
  ...
  constructor(private route: ActivatedRoute, private mapService: MapService, public teamService: TeamService, private userService: UserService) {
    ...
    let userCoords = this.userService.userCoords;
    this.lat = 0;
    this.lng = 0;
    ...
    this.teamService.downloadTeams()
      .then((teams: any[]) => {
        ...
        this.sub = this.route.params.subscribe(params => {
          ...
          if (teamId) {
            for (let k = 0; k < this.allTeams.length; k++) {
          ...
          if (locId) {
            const loc = this.mapService.getMarkerById(locId);
  ...
  */
  submit() {
    ...
    if (this.formIsValid()) {
      ...
      let usersTeam = this.teamService.getTeamById(this.selectedUserTeam);
      ...
      let sport = usersTeam.sport;

      let players = usersTeam.members.concat(oppTeam.members);
